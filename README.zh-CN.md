...
- `docs/report-schema.md`：JSON 报告字段契约和消费方说明。
- `docs/model-integrity-methodology.md`：模型完整性控制项和限制说明。
- `examples/reports/`：用于快速理解和下游集成的去敏感化示例报告。
...
参见 `docs/report-schema.md` 了解 JSON 字段契约，参见 `examples/reports/` 查看去敏感化示例报告。

参见 `docs/model-integrity-methodology.md` 了解正向控制、负向控制、model echo、usage visibility 和 baseline similarity 的设计理由。
...
应结合 `verdict`、`score`、`warnings` 和 `failed_controls` 一起判断。高分表示端点在本次运行中符合当前实现的行为控制项，但不是后端身份的加密学证明。

复查信号可能有多种原因：

- 缺失 `usage` 可能只是 relay 隐藏了元数据，不一定代表模型是假的。
- Model echo mismatch 可能来自代理归一化、别名、fallback routing 或错误路由。
- Timeout、损坏 JSON 或 server error 是可靠性信号，通常应重新运行后再下强结论。
- 与基线模型高度相似只是行为证据，应结合负向控制和 HTTP 证据一起解读。

如果用于账单、采购或事故响应判断，应结合供应商日志、账单导出、官方端点对比和独立运维检查。
